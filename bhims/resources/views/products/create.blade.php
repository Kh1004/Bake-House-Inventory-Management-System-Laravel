            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="mt-1 block w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <select name="recipe_id" id="recipe_id"
                        ...
                        @foreach($recipes as $id => $name)
                            <option value="{{ $id }}" {{ old('recipe_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                    @error('recipe_id')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
    ...
    if (nameInput && skuInput && !skuInput.value) {
        nameInput.addEventListener('blur', function() {
            if (!skuInput.value) {
                ...
                if (name) {
                    // Generate a simple SKU from the name (first 3 letters of each word, uppercase)
                    const sku = name.split(' ')
                        .map(word => word.substring(0, 3).toUpperCase())
                        .join('');
                    skuInput.value = sku + Math.floor(100 + Math.random() * 900); // Add random 3 digits
                }
            }
        });
    }

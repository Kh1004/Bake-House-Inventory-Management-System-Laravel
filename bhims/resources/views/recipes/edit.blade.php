...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-2xl font-bold text-gray-800">Edit Recipe: {{ $recipe->name }}</h1>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <form action="{{ route('recipes.update', $recipe) }}" method="POST" id="recipe-form">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="serving_size" id="serving_size" min="0.1" step="0.1" required
                                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                value="{{ old('serving_size', $recipe->serving_size) }}">
                            @error('serving_size')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="selling_price" id="selling_price" min="0" step="0.01" required
                                    class="pl-7 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div class="flex items-end">
                            <div class="flex items-center h-10">
                                ...
                                <input type="checkbox" name="is_active" id="is_active" value="1" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" {{ (old('is_active', $recipe->is_active) ? 'checked' : '') }}>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="2"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description', $recipe->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="instructions" id="instructions" rows="6" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('instructions', $recipe->instructions) }}</textarea>
                        @error('instructions')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-8">
                        ...
                        <div id="ingredients-container" class="space-y-4">
                            @php
                                ...
                                // If there's old input, use that, otherwise use the recipe's ingredients
                                if (!empty($oldIngredients)) {
                                    foreach ($oldIngredients as $index => $ing) {
                                        $ingredientsData[] = [
                                            'id' => $ing['id'] ?? null,
                                    ...
                                    foreach ($recipe->ingredients as $ingredient) {
                                        $ingredientsData[] = [
                                            ...
                                            'unit_of_measure' => $ingredient->pivot->unit_of_measure,
                ...
                <div class="px-6 py-3 bg-gray-50 text-right">
                    <a href="{{ route('recipes.show', $recipe) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Cancel
                    </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        let ingredientIndex = {{ count($ingredientsData) }};
                    ...
                    <div class="md:col-span-5">
                        ...
                        <select name="ingredients[${ingredientIndex}][id]" required
                            ...
                            @foreach($ingredients as $ingredient)
                                <option value="{{ $ingredient->id }}" data-unit="{{ $ingredient->unit_of_measure }}">
                                    {{ $ingredient->name }}
                                </option>
                            @endforeach
                        </select>
